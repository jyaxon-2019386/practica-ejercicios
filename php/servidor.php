...
switch ($request_method) {
    ...
    case 'POST':
        ...
        $nombre = $con->real_escape_string($data["nombre"]);
            ...
            if (!empty($usuario) && !empty($contrasena)) {
                ...
                if ($rs_verify && $rs_verify->num_rows > 0) {
                    ...
                    if (password_verify($contrasena, $hashed_password_verify)) {
                        $user_id = $user_verify['id']; // Obtén el ID del usuario
                        ...
                        if ($usuario_nuevo !== $usuario) {
                            ...
                            do {
                                ...
                                $rs = mysqli_query($con, $user_check);
                                if (mysqli_num_rows($rs) > 0) {
                                    // Si el usuario ya existe, incrementa y genera uno nuevo
                                    $incremento++;
                            ...
                            if ($usuario_generado !== $usuario_nuevo) {
                                // El usuario ya existe, sugerimos uno nuevo
                                $response = [
                                    "success" => false,
                                    "message" => "El usuario ya existe. Intenta con este nuevo nombre: $usuario_generado"                                ];
                                header('Content-Type: application/json');
                                echo json_encode($response);
                                exit(); // Salimos para que el usuario decida si quiere aceptar la sugerencia
                            }
        
                            $usuario = $usuario_generado; // Asigna el nombre de usuario generado
                        }
        
                        $update_fields = [];
                        if (!empty($nombre_nuevo)) $update_fields[] = "nombre = '$nombre_nuevo'";
                        if (!empty($usuario_nuevo)) $update_fields[] = "usuario = '$usuario'";
                        if ($hashed_password !== $hashed_password_current) $update_fields[] = "contrasena = '$hashed_password'";
        
                        if (!empty($update_fields)) {
                            // Cambiamos a usar el id para el WHERE
                            ...
                            if ($response_sql) {
                                $response = [
                                    ...
                                    "usuario" => $usuario, // El usuario nuevo o generado
